...
func init() {
	android.RegisterModuleType("catalyst_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = catalystExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = catalystExpandVariables(ctx, dep_file)
	...
	cmd := catalystExpandVariables(ctx, String(g.properties.Cmd))
